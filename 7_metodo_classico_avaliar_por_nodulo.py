# =============================================================================
# 7_metodo_classico_avaliar_por_nodulo.py — Avaliação por NÓDULO (não por slice)
# =============================================================================
#
# Diferente do script 6 (métricas por slice/pixel), este script avalia a
# DETECÇÃO em nível de nódulo físico, no volume 3D completo de cada
# paciente de teste:
#
#   - Nódulo ENCONTRADO: existe sobreposição entre alguma região prevista
#     pelo método e a máscara de consenso daquele nódulo (3D).
#   - Dice do nódulo: calculado só para os nódulos ENCONTRADOS, entre a
#     máscara de consenso e a união das regiões previstas que o tocam.
#   - Falso positivo: região prevista que não corresponde a nenhum nódulo
#     válido real — contada à parte, NUNCA entra na média de Dice.
#   - Nódulo perdido (não encontrado): fica de fora da média de Dice
#     (não tem o que medir — não há sobreposição nenhuma).
#
# Saída: metricas_por_nodulo_teste.csv (1 linha por nódulo real) +
#        resumo agregado impresso no console.
#
# =============================================================================

import os
import json
import numpy as np
import pandas as pd
import SimpleITK as sitk
from scipy import ndimage
from importlib.machinery import SourceFileLoader

# --- Configurações ---

CAMINHO_SAIDA = r"D:\TG\processadoV3"
CAMINHO_SLICES = r"D:\TG\slices_2dV3"
CAMINHO_SPLIT = os.path.join(CAMINHO_SAIDA, "split_pacientes.csv")
CAMINHO_PARAMS = os.path.join(CAMINHO_SLICES, "melhores_parametros.json")
CAMINHO_RESULTADOS = os.path.join(CAMINHO_SLICES, "metricas_por_nodulo_teste.csv")

CAMINHO_SCRIPT_5 = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "5_metodo_classico_calibrar.py")
_mod5 = SourceFileLoader("metodo_classico", CAMINHO_SCRIPT_5).load_module()

HU_MIN, HU_MAX = _mod5.HU_MIN, _mod5.HU_MAX

# Conectividade 3D usada para agrupar voxels previstos em "candidatos"
# (26-conectividade — qualquer voxel vizinho, incl. diagonais, é o mesmo
# candidato; evita fragmentar um único nódulo previsto em vários pedaços
# por causa de leves descontinuidades entre slices).
ESTRUTURA_3D = ndimage.generate_binary_structure(3, 3)


def normalizar_hu(volume_hu):
    clip = np.clip(volume_hu, HU_MIN, HU_MAX)
    return ((clip - HU_MIN) / (HU_MAX - HU_MIN)).astype(np.float32)


def dice(a, b):
    a = a.astype(bool); b = b.astype(bool)
    inter = np.logical_and(a, b).sum()
    soma = a.sum() + b.sum()
    return 1.0 if soma == 0 else 2.0 * inter / soma


def carregar_mascara_nodulo(pasta_paciente, cluster_id, shape,
                             metodo_preferido="staple"):
    """Máscara de consenso de UM nódulo específico (não união de vários)."""
    ordem = [metodo_preferido,
             "votacao" if metodo_preferido == "staple" else "staple"]
    for metodo in ordem:
        cam = os.path.join(
            pasta_paciente, "mascara_consenso_nodulo%02d_%s.nii.gz" % (
                cluster_id, metodo))
        if os.path.exists(cam):
            return sitk.GetArrayFromImage(sitk.ReadImage(cam)).astype(bool)
    return np.zeros(shape, dtype=bool)


def avaliar_paciente(paciente, cluster_ids_validos, params):
    pasta_paciente = os.path.join(CAMINHO_SAIDA, paciente)
    cam_volume = os.path.join(pasta_paciente, "volume_tc.nii.gz")
    if not os.path.exists(cam_volume):
        print("  [ERRO] volume não encontrado para %s — pulando" % paciente)
        return [], 0

    volume_hu = sitk.GetArrayFromImage(sitk.ReadImage(cam_volume))
    volume_norm = normalizar_hu(volume_hu)
    shape = volume_hu.shape

    # Roda o método clássico em TODO slice do volume (às cegas)
    pred_3d = np.zeros(shape, dtype=bool)
    for z in range(shape[0]):
        pulmao = _mod5.segmentar_pulmao(volume_norm[z])
        pred_3d[z] = _mod5.segmentar_nodulos_classico(
            volume_norm[z], pulmao, **params).astype(bool)

    # Agrupa os voxels previstos em candidatos (componentes conexos 3D)
    rotulado_pred, n_candidatos = ndimage.label(pred_3d, structure=ESTRUTURA_3D)

    linhas = []
    rotulos_correspondidos = set()

    for cluster_id in cluster_ids_validos:
        gt_nodulo = carregar_mascara_nodulo(pasta_paciente, cluster_id, shape)
        if gt_nodulo.sum() == 0:
            continue  # máscara de consenso não encontrada nesse paciente

        rotulos_sobrepostos = set(
            np.unique(rotulado_pred[gt_nodulo & (rotulado_pred > 0)]))
        rotulos_sobrepostos.discard(0)

        encontrado = len(rotulos_sobrepostos) > 0
        if encontrado:
            rotulos_correspondidos |= rotulos_sobrepostos
            pred_correspondente = np.isin(rotulado_pred, list(rotulos_sobrepostos))
            dice_nodulo = dice(pred_correspondente, gt_nodulo)
        else:
            dice_nodulo = np.nan

        linhas.append({
            "paciente": paciente, "cluster_id": cluster_id,
            "encontrado": encontrado, "dice": dice_nodulo,
            "volume_gt_voxels": int(gt_nodulo.sum()),
        })

    # Falsos positivos: candidatos previstos que não tocam NENHUM nódulo
    # válido real — nunca entram na média de Dice.
    todos_rotulos = set(range(1, n_candidatos + 1))
    falsos_positivos = len(todos_rotulos - rotulos_correspondidos)

    return linhas, falsos_positivos


def main():
    with open(CAMINHO_PARAMS) as f:
        params = json.load(f)
    print("Parâmetros carregados:", params)

    split_df = pd.read_csv(CAMINHO_SPLIT)
    pacientes_teste = split_df[split_df["split"] == "test"]["paciente"].tolist()
    print("Pacientes de teste: %d" % len(pacientes_teste))

    nodulos_df = pd.read_csv(os.path.join(CAMINHO_SAIDA, "nodulos_validos.csv"))
    clusters_por_paciente = (
        nodulos_df.groupby("paciente")["cluster_id"].unique().apply(list).to_dict()
    )

    todas_linhas = []
    total_fp = 0
    for paciente in pacientes_teste:
        cluster_ids = clusters_por_paciente.get(paciente, [])
        linhas, fp = avaliar_paciente(paciente, cluster_ids, params)
        todas_linhas.extend(linhas)
        total_fp += fp
        n_encontrados = sum(1 for l in linhas if l["encontrado"])
        print("  %s: %d nódulo(s) real(is) | %d encontrado(s) | "
              "%d falso(s) positivo(s)" % (
                  paciente, len(linhas), n_encontrados, fp))

    df = pd.DataFrame(todas_linhas)
    df.to_csv(CAMINHO_RESULTADOS, index=False)

    n_total = len(df)
    n_encontrados = int(df["encontrado"].sum())
    n_perdidos = n_total - n_encontrados
    dice_medio = df.loc[df["encontrado"], "dice"].mean()

    print("\n" + "=" * 60)
    print("Resumo — avaliação por NÓDULO (todos os pacientes de teste)")
    print("=" * 60)
    print("  Nódulos reais no teste (válidos):  %d" % n_total)
    print("  Nódulos ENCONTRADOS:               %d (%.1f%%)" % (
        n_encontrados, 100 * n_encontrados / n_total if n_total else 0))
    print("  Nódulos PERDIDOS (não detectados): %d (%.1f%%)" % (
        n_perdidos, 100 * n_perdidos / n_total if n_total else 0))
    print("  Falsos positivos (candidatos sem nódulo real correspondente): %d" %
          total_fp)
    print("  Dice médio — SÓ dos nódulos encontrados (sem contar FP): %.4f" %
          dice_medio)
    print("\nResultados salvos em: %s" % CAMINHO_RESULTADOS)


if __name__ == "__main__":
    main()
